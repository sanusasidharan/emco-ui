  ...
  const handleClose = (values) => {
    if (values) {
      ...
      apiService
        ...
        .then((res) => {
          if (request.metadata.name === "dtc") {
            request = {
              metadata: { name: "its", description: "its sub controller" },
              spec: {
                host: "its",
                port: 9040,
                type: "action",
            ...
            apiService.addITSController(request).then(res=>{}).catch(err=>{
              console.log(err)
            });
          }
        ...
        .catch((err) => {
          ...
          setNotificationDetails({
            show: true,
            message: "Error adding controller : " + err,
            severity: "error",
          });
        });
    }
      ...
      {!isLoading && (
        <>
          ...
          <ControllerForm
            open={open}
            onClose={handleClose}
            existingControllers={controllersData}
          />
